...
from blocks.Embed import positional_encoding
...
class TSTEncoder(nn.Module):
    ...
    def forward(self, src:Tensor,z, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None):
        output = src
        scores = None
        ...
        for mod in self.layers:
            ...
            i=i+1
            if i==3:
                feature=output.clone().requires_grad_(True)
                if z is not None:
                        output = output + z
                else:
                        output =output +torch.zeros_like(output ).to(output .device)
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    def __init__(self, configs, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        if not isinstance(d_model,int):
            d_model = d_model[configs.data]
        if not isinstance(n_heads,int):
            n_heads = n_heads[configs.data]
    ...
    def forward(self, x,z) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        n_vars = x.shape[1]
        # Input encoding
        ...
        # Encoder
        z,feature = self.encoder(u,z)                                                      # z: [bs * nvars x patch_num x d_model]
...
class PatchTST_backbone(nn.Module):
    def __init__(self, configs, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        if not isinstance(n_layers, int):
            n_layers=n_layers[configs.data]

        if not isinstance(n_heads, int):
            n_heads=n_heads[configs.data]

        if not isinstance(d_ff, int):
            d_ff=d_ff[configs.data]
    ...
    def forward(self, z,z_):    
        ...
        # model
        z,feature = self.backbone(z,z_) # z: [bs x nvars x d_model x patch_num] 32, 321, 128, 12
...
class Model(nn.Module):
    ...
    def forward(self, x,zs,zt):           # x: [Batch, Input length, Channel]
        if zs != None:
            if len(zs.shape)==2:
                b=x.shape[0]
                # z=z[:b]
                zs=zs.unsqueeze(0).repeat(b,1,1)
            if len(zs.shape)==3:
                b=x.shape[0]
                zs=zs[:b]
        if zt !=None:
            if len(zt.shape)==2:
                b=x.shape[0]
                # z=z[:b]
                zt=zt.unsqueeze(0).repeat(b,1,1)
            if len(zt.shape)==3:
                b=x.shape[0]
                zt=zt[:b]
        if self.decomposition:
            featuret,features=None,None
            res_init, trend_init = self.decomp_module(x)
            res_init, trend_init = res_init.permute(0,2,1), trend_init.permute(0,2,1)  # x: [Batch, Channel, Input length]
            res,features = self.model_res(res_init,zs)
            trend,featuret = self.model_trend(trend_init,zt)
            x = res + trend
