...
class Leddam(nn.Module): 
    def __init__(self,
        ...
        self.position_embedder=DataEmbedding(pe_type=pe_type,seq_len=seq_len, 
                                            d_model=d_model,c_in=enc_in,dropout=dropout) 
...
class Model(nn.Module):
    def __init__(self, configs,_):
        ...
        self.leddam=Leddam(configs['enc_in'],dropout=configs['dropout'])
        ...
        self.Linear_res.weight = nn.Parameter(
                (1 / configs['d_model']) * torch.ones([configs['pred_len'], configs['d_model']])) 
    def forward(self, inp,zs=None,zt=None):
        features,featuret=None,None
        ...
        if zs is not None:
            if len(zs.shape)==3:
                zs=zs[:inp.shape[0],:,:]
            if len(zs.shape)==2:
                zs=zs.unsqueeze(0).repeat(inp.shape[0],1,1)
            features = res_out.clone().requires_grad_(True).permute(0,2,1)
                
            res_out = res_out+zs.permute(0,2,1)
        if zt is not None:
            if len(zt.shape)==3:
                zt=zt[:inp.shape[0],:,:]
            if len(zt.shape)==2:
                zt=zt.unsqueeze(0).repeat(inp.shape[0],1,1)
            featuret = main_out.clone().requires_grad_(True).permute(0,2,1)
            main_out = main_out+zt.permute(0,2,1)
        pred=main_out+res_out
        if self.revin:
            pred=self.revin_layer.inverse_normalize(pred)
       
        return {'pred':pred,'features':features,'featuret':featuret}