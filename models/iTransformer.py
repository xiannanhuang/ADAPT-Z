...
class Model(nn.Module):
    ...
    def __init__(self, configs, input_len):
        ...
        if not isinstance(configs.e_layers,int):
            configs.e_layers=configs.e_layers[configs.data]
        # if not isinstance(configs.n_heads,int):
        #     configs.n_heads=configs.n_heads[configs.data]
        if not isinstance(configs.d_ff,int):
            configs.d_ff=configs.d_ff[configs.data]
        
        # Embedding
        self.enc_embedding = DataEmbedding_inverted(self.seq_len, d_model, configs.embed, 'h',
                                                    configs.dropout)
