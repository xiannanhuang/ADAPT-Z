...
class FSConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, groups=1, gamma=0.9):
        ...
        self.grads = torch.Tensor(sum(self.grad_dim)).fill_(0).to(device='cuda:0')
        self.f_grads = torch.Tensor(sum(self.grad_dim)).fill_(0).to(device='cuda:0')
        nh=64
    ...
    def fw_chunks(self):
        ...
        if not hasattr(self, 'q_ema'):
            setattr(self, 'q_ema', torch.zeros(*q.size()).float().to(device='cuda:0'))
        else:
