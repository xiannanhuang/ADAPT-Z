from models.encoders.dilatedFSConvEnc import DilatedFSConvEnc
...
class TSEncoder(nn.Module):
    def __init__(self, input_dims, output_dims, args, hidden_dims=64, depth=10, mask_mode='binomial', ):
        ...
        super().__init__()
        self.input_dims = args.enc_in
        self.output_dims = output_dims
        self.hidden_dims = hidden_dims
        self.mask_mode = mask_mode
        self.input_fc = nn.Linear(args.enc_in, hidden_dims)
    ...
    def forward(self, x, mask=None):  # x: B x T x input_dims
        ...
        nan_mask = ~x.isnan().any(axis=-1)
        x[~nan_mask] = 0
        x = self.input_fc(x[...,:self.input_dims])  # B x T x Ch
