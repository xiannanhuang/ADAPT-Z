...
class Ada_fineture:
    ...
    def bulid_adapter(self,d_model,args):
        class Adapter(nn.Module):
            def __init__(self,d_model,args):
                ...
                self.linear3=nn.Linear(64,d_model)
    ...
    def online(self, test_loader,online=True):
        pred=[]
        truth=[]
        x_list=[]
        self.optimizer2=optim.Adam(self.model.projector.parameters(),lr=settings[self.args.model][self.args.data]['online_lr'])
...
def finetune(model=None,data=None,data_path=None,seq_len=None,pred_len=None,seed=None,ogd_step=0,ema=0.7):
    ...
    mae,mse=adapter.online(test_loader,online=False)
    print(f'测试集mae:{mae},mse:{mse}')
    res={'model':args.model,'data':args.data,'seq_len':args.seq_len,'pred_len':args.pred_len,'seed':args.seed,'mae':mae,'mse':mse}
    res=pd.DataFrame(res,index=[0])
    output_path = "itrans_trr.csv"
    res.to_csv(output_path, mode='a', header=not os.path.exists(output_path), index=False)
    
    return mae,mse 
import itertools
for model,data,pred_len,seed in itertools.product(['iTransformer'],['ETTh1','ETTh2','ETTm1','electricity','ETTm2','traffic','PEMS03',
                                                                                         'PEMS04','weather','PEMS07','PEMS08','solar','exchange'],[1,24,48],[2025]):
    # df=pd.read_csv('itrans_tr1.csv')
    # done_experiments = df[['pred_len','data','seq_len','seed']].drop_duplicates()
    ...
    # if f'{pred_len}_{data}_{96}_{seed}' in done_experiments:
    #         print(f'{pred_len}_{data}_{96}_{seed} already done')
    # else:
        finetune(model,data=data,pred_len=pred_len,seed=seed)