...
import random,yaml
import pandas as pd
from models.DishTS_model import Model
from models.blocks.DishTS import DishTS
import numpy as np
...
def main(model=None,data=None,data_path=None,seq_len=None,pred_len=None,seed=None):
    ...
    output_path = "experiment_itrans_dish.csv"
    res.to_csv(output_path, mode='a', header=not os.path.exists(output_path), index=False)
if __name__ == '__main__':
    import itertools
    for model,seed,data,pred_len in itertools.product(['iTransformer'],[2025],['traffic','weather','PEMS07','PEMS08','exchange'],[1,24,48]):
